...
workspaceInviteRoute.get("/w/:slug/invite/:code", requireUser, async (c) => {
  try {
    ...
    if (!workspace) {
      return renderPage(c, InvitePage({ error: "Workspace not found" }), 404);
    }
    ...
    if (!invite) {
      return renderPage(c, InvitePage({ error: "Invalid invite link" }), 404);
    }
    ...
    return renderPage(c, InvitePage({ workspace, code }));
  } catch (error) {
    console.error("Error loading invite:", error);
    return renderPage(c, InvitePage({ error: "Failed to load invite" }), 500);
  }
});

workspaceInviteRoute.post("/w/:slug/invite/:code", requireUser, async (c) => {
  try {
    const db = getDb(c.env.DB);
    const user = c.get("user");
    const slug = c.req.param("slug");
